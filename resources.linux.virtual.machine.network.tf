...
resource "azurerm_public_ip" "pip" {
  ...
  tags                = merge(local.default_tags, var.add_tags, var.public_ip_add_tags)
...
resource "azurerm_network_interface" "nic" {
  ...
  tags                          = merge(local.default_tags, var.add_tags, var.nic_add_tags)
...
resource "azurerm_proximity_placement_group" "appgrp" {
  ...
  name                = lower("${local.vm_name}-${var.location}-proxigrp")
  resource_group_name = local.resource_group_name
  location            = local.location
  tags                = merge({ "ResourceName" = lower("${local.vm_name}-${var.location}-proxigrp") }, var.tags, )
...
resource "azurerm_availability_set" "aset" {
  ...
  managed                      = true
  tags                         = merge({ "ResourceName" = lower(local.vm_avset_name) }, var.tags, )
...
resource "azurerm_network_interface_security_group_association" "nsgassoc" {
  count                     = var.attach_existing_network_security_group ? 1 : 0
